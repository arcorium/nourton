...
namespace ar
{
	...
	public:
		...
		/**
		 * encrypt single block, it will only return error message when the cipher_block is not 16 bytes long
		 * @param block text with 16 bytes long
		 * @return 16 bytes long cipher text or error message
		 */
		[[nodiscard]] std::expected<std::array<u8, KEY_BYTE>, std::string_view> encrypt(block_type block) const noexcept;

		/**
		 * encrypt arbitrary bytes using EBC Mode. when the bytes is not multiple of 16 it will filled with garbage bytes to fulfill it
		...
		/**
		 * decipher cipher text and only return error message when the bytes parameter size is not multiple of 16
		 * @param bytes cipher text with multiple of 16 size
		 * @param garbage helper to remove the garbage bytes, when this parameter is not provided no bytes will be removed
