...
namespace ar
{
  ...
  boost::multiprecision::number<boost::multiprecision::cpp_int_backend<
      Bits, Bits, SignType, Checked, void>> static rawToBoost(const void* V)
  {
  ...
  template <typename To, typename From>
    requires requires { sizeof(To) == 2 * sizeof(From); }
  ...
  template <typename T, typename U>
  constexpr std::span<const T, sizeof(U) / sizeof(T)> as_span(const U& data_) noexcept
  {
    return std::span<const T, sizeof(U) / sizeof(T)>{reinterpret_cast<const T*>(&data_),
                                                     sizeof(U) / sizeof(T)};
  }
  ...
  constexpr std::array<u8, 16> as_bytes(const u128& val) noexcept
  {
    ...
    {
      auto p = reinterpret_cast<limb_type*>(bytes.data() + i * sizeof(limb_type));
      *p = *limbs;
